...
@login_required
def canvas_list():
    return get_json_result(data=sorted([c.to_dict() for c in UserCanvasService.query(user_id=current_user.id)], key=lambda x: x["update_time"] * -1))
...
def list_kbs():
    ...
    try:
        ...
        kbs, total = UserCanvasService.get_by_tenant_ids([m["tenant_id"] for m in tenants], current_user.id, page_number, items_per_page, orderby, desc, keywords)
